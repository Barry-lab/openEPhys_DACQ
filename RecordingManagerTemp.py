...
def store_camera_data_to_recording_file(camera_settings, fpath):
    ...
    for cameraID in camera_settings['CameraSpecific'].keys():
        ...
        print(
            ['DEBUG', hfunct.time_string(), 'Initializing Camera_RPi_file_manager for ID: ' + cameraID + ' complete.'])
    ...
    camera_data = {}
    for cameraID in camera_settings['CameraSpecific'].keys():
        print(['DEBUG', hfunct.time_string(), 'Retrieving camera_data for ID: ' + cameraID])
        camera_data[cameraID] = file_managers[cameraID].get_timestamps_and_OnlineTrackerData(cameraID)
        print(['DEBUG', hfunct.time_string(), 'Retrieving camera_data for ID: ' + cameraID + ' complete.'])
    print(['DEBUG', hfunct.time_string(), 'Saving camera_data to recording file'])
    NWBio.save_tracking_data(fpath, camera_data)
    print(['DEBUG', hfunct.time_string(), 'Saving camera_data to recording file complete.'])
...
class TestDevicesError(Exception):
    pass
...
class RecordingManager(object):
    ...
    def test_ping_of_ip_addresses(ip_addresses):
        ...
        def test_ping_and_update_locked_list(ip_address, locked_list, list_pos, list_lock):
            ret = hfunct.test_pinging_address(ip_address)
            with list_lock:
    ...
    def test_devices(self):
        ...
        if self.general_settings['Tracking']:
            if self.settings_ready['camera_settings']:
                ...
                raise TestDevicesError('Tracking is turned on, but Camera Settings are not available.')
        ...
        if self.general_settings['TaskActive']:
            if self.settings_ready['task_settings']:
                ...
                raise TestDevicesError('Task is turned on, but Task Settings are not available.')
    ...
    def check_if_ready_to_initialize(self):
        ...
        if self.general_settings['TaskActive']:
            if not self.general_settings['Tracking']:
                raise TestDevicesError('Tracking must be turned on')
    ...
    def start_rec(self):
        ...
        recording_folder_root = os.path.join(self.general_settings['root_folder'], str(self.pt_animal.toPlainText()))
        ...
        if self.general_settings['Tracking']:
            ...
            self.position_plot = PosPlot(self.online_tracking_processor, self.camera_settings['General']['LED_angle'])
...
class RecordingManagerGUI(QtWidgets.QMainWindow):
    ...
    def save_settings_button_callback(self):
        fpath = hfunct.openSingleFileDialog('save', suffix='nwb', caption='Save file name and location')
        if not (fpath is None):
            self.recording_manager.save_settings(fpath)

    def load_settings_button_callback(self):
        fpath = hfunct.openSingleFileDialog('load', suffix='nwb', caption='Select file to load')
        if not (fpath is None):
