...
class Core(object):
    def __init__(self, TaskSettings, TaskIO):
        ...
        # Prepare rewards
        self.PelletReward = []
        for n_Pfeeder in self.TaskSettings['UseFeeders']:
    ...
    def game_logic(self):
        ...
        # Check if enough time as passed since last reward
        timeSinceLastReward = time.time() - self.lastPelletReward
        rewardDelayCriteria = {'name': 'Since Reward', 
                               'target': self.TaskSettings['RewardMinSeparation'], 
                               'status': int(round(timeSinceLastReward)), 
                               'complete': timeSinceLastReward >= self.TaskSettings['RewardMinSeparation'], 
                               'percentage': timeSinceLastReward / float(self.TaskSettings['RewardMinSeparation'])}
        ...
        for gp in game_progress:
            status_complete.append(gp['complete'])
        if all(status_complete):
            for n_Pfeeder in self.TaskSettings['UseFeeders']:
                self.releasePellet(n_Pfeeder, actor='GameEvent', n_pellets=1)
